 ...
 * FotPath Defintions
 **/
PyObject *FotPath_new(PyTypeObject *type, PyObject *args, PyObject *kwds) {
    FotPath *self = (FotPath *)type-> tp_alloc(type, 1);
    self->path = NULL; // set path = nullptr
    printf("Module: Path new\n");
    return (PyObject *) self;
}

void FotPath_dealloc(FotPath *self) {
    printf("Module: path dealloc\n");
    delete self->path;
...
PyTypeObject FotPathType = {
    PyVarObject_HEAD_INIT(NULL, 0)
    .tp_name = "fot_planner.FotPath",
    .tp_basicsize = sizeof(FotPath),
    .tp_dealloc = (destructor)FotPath_dealloc,
    // .tp_repr = (reprfunc)FotPath_repr, // we can do something here
    .tp_flags = Py_TPFLAGS_DEFAULT |
    Py_TPFLAGS_BASETYPE,
    .tp_doc = "fot_planner.FotPath objects",
