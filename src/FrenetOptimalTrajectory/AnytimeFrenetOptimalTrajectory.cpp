...
void AnytimeFrenetOptimalTrajectory::asyncPlan() {
    ...
    // vector<thread> threads; // declared in .h, no need to declare again
    ...
    for (int i = 0; i < fot_hp->num_threads; i++) {
        if (i != fot_hp->num_threads - 1) {
            threads->push_back(thread(
                &AnytimeFrenetOptimalTrajectory::calc_frenet_paths_threaded, this,
                i * iter_di_index_range, (i + 1) * iter_di_index_range));
        } else { // account for last thread edge case
            threads->push_back(
                thread(&AnytimeFrenetOptimalTrajectory::calc_frenet_paths_threaded,
                        this, i * iter_di_index_range, num_di_iter));
        }
    }

    printf("all threads added\n");
 ...
 */
void AnytimeFrenetOptimalTrajectory::calc_frenet_paths_threaded(int start_di_index,
                                                         int end_di_index) {

    // mu->lock();
    // bool worker_state = run_workers;
    // mu->unlock();
